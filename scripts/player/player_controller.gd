...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			var targets: Array[Dictionary] = find_targets(event)
			if targets.size() > 0:
				...
				if collider.get_collision_layer_value(3) and collider.get_collision_layer_value(5):
					target_node = collider as Node2D
					rot_pos = target_node.position
					rotate_pov()
					is_attacking = true
			else:
...
func find_targets(event: InputEventMouseButton) -> Array[Dictionary]:
...
func fire_bullet(node: Node2D):
	var player_bullet: PlayerBullet = player_bullet_scene.instantiate()
	player_bullet = player_bullet.dmg(damage).dont_collide_with(self).dont_collide_with_player().set_target(node).set_start_position(self.global_position).timeout(4.0)
